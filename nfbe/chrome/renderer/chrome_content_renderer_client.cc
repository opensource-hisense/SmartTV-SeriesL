...
bool ChromeContentRendererClient::OverrideCreatePlugin(
  ...
  if (orig_mime_type == "application/x-oipf-video"
      || (f_document_is_hbbtv_document && orig_mime_type == "video/mp4")) {
    int id = IDR_OIPF_VIDEO_PLUGIN_HTML;
    // id = IDR_MOBILE_YOUTUBE_PLUGIN_HTML;
...
#if defined(HBBTV_ON)
  {
    ...
    // for the values of enum class ChromeViewHostMsg_GetPluginInfo_Status.
    ChromeViewHostMsg_GetPluginInfo_Status status = output.status;
    VLOG(X_LOG_V) << "ZZZZ ChromeContentRendererClient::OverrideCreatePlugin:"
                  << " GetPluginInfo_Output.status=" << static_cast<int>(status)
      ;
  }
#endif
...
#if defined(HBBTV_ON)
    {
      VLOG(X_LOG_V) << "ZZZZ: ChromeContentRendererClient::CreateBrowserPluginDelegate:"
                    << " mime_type=" << mime_type
        ;
    }
#endif
