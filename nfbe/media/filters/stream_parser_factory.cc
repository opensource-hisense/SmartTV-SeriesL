...
#if defined(ENABLE_EXTERNAL_ISTYPESUPPORTED)
...
#if defined(ENABLE_YOUTUBE_ISTYPESUPPORTED_EXTENSIONS)
...
#if defined(ENABLE_EXTERNAL_ISTYPESUPPORTED)
#if defined(ENABLE_YOUTUBE_ISTYPESUPPORTED_EXTENSIONS)
  return peer::media::MediaIsSupportedMediaSourceTypePeer(
      type, codecs, width, height, framerate, bitrate, channels, eotf,
      NULL, NULL, NULL);
#else
  return peer::media::MediaIsSupportedMediaSourceTypePeer(
      type, codecs, 0, 0, 0, 0, 0, "", NULL, NULL, NULL);
#endif
#else
...
scoped_ptr<StreamParser> StreamParserFactory::Create(
  ...
  *has_video = false;
  ...
  (void)CheckTypeAndCodecs;
  peer::media::ParserType parser_type = peer::media::NO_PARSER;
  if (peer::media::MediaIsSupportedMediaSourceTypePeer(type, codecs,
      0, 0, 0, 0, 0, "", &parser_type, has_audio, has_video)) {
    switch (parser_type) {
    case peer::media::WEBM_PARSER:
      stream_parser.reset(BuildWebMParser(codecs, media_log));
      break;
#if defined(USE_PROPRIETARY_CODECS)
    case peer::media::ADTS_PARSER:
      stream_parser.reset(BuildADTSParser(codecs, media_log));
      break;
    case peer::media::MP3_PARSER:
      stream_parser.reset(BuildMP3Parser(codecs, media_log));
      break;
    case peer::media::MP4_PARSER:
      stream_parser.reset(BuildMP4Parser(codecs, media_log));
      break;
    case peer::media::MP2T_PARSER:
#if BUILDFLAG(ENABLE_MSE_MPEG2TS_STREAM_PARSER)
      stream_parser.reset(BuildMP2TParser(codecs, media_log));
#endif
#endif
      break;
    default:
