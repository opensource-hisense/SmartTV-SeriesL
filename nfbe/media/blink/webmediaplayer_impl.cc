...
void WebMediaPlayerImpl::OnGeometryMaybeChanged() {
  ...
  WebSize scrollOffset = frame_->scrollOffset();

  // Compute rectangle in screen
  float x = rect.x() - scrollOffset.width;
  float y = rect.y() - scrollOffset.height;
  float width = rect.width();
  float height = rect.height();

  gfx::RectF hole_rect(x, y, width, height);

  if (hole_rect != last_computed_rect_) {
    last_computed_rect_ = hole_rect;
    pipeline_.UpdateVideoHoleRect(last_computed_rect_);
  }
}
