...
#if defined(HBBTV_ON)
    // if this is an XML AIT pars the relevant fields from it
    const char* XMLAIT_TAG_ORG_ID = "orgId";
    const char* XMLAIT_TAG_APP_ID = "appId";
    const char* XMLAIT_TAG_CONTROL_CODE = "controlCode";
    const char* XMLAIT_TAG_VISIBILITY =  "visibility";
    const char* XMLAIT_TAG_SERVICE_BOUND = "serviceBound";
    const char* XMLAIT_TAG_PRIORITY = "priority";
    const char* XMLAIT_TAG_URL_BASE = "URLBase";
    const char* XMLAIT_TAG_APP_OCATION = "applicationLocation";

    if (document()->isXMLAITDocument() && m_currentNode->isElementNode()) {
        Element* e = toElement(m_currentNode);
        AtomicString localName = e->localName();
        String contentStr((char*)chars, length);
        if (localName == XMLAIT_TAG_ORG_ID) {
            document()->setAITOrgID(contentStr.toUInt());
        } else if (localName == XMLAIT_TAG_APP_ID) {
            document()->setAITAppID(contentStr.toUInt());
        } else if (localName == XMLAIT_TAG_CONTROL_CODE) {
            document()->setAITControlCode(contentStr);
        } else if (localName == XMLAIT_TAG_VISIBILITY) {
            document()->setAITVisibility(contentStr);
        } else if (localName == XMLAIT_TAG_SERVICE_BOUND) {
            document()->setAITServiceBound(contentStr.upper() == "TRUE");
        } else if (localName == XMLAIT_TAG_PRIORITY) {
            document()->setAITPriority((unsigned char)contentStr.toInt());
        } else if (localName == XMLAIT_TAG_URL_BASE) {
            document()->setAITUrlBase(contentStr);
        } else if (e->localName() == XMLAIT_TAG_APP_OCATION) {
            document()->setAITApplicationLocation(contentStr);
        }
        return;
    }
#endif
