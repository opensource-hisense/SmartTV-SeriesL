...
#if defined(HBBTV_ON)
bool LayoutEmbeddedObject::hasContentsLayer() const
{
    Widget* widget = this->widget();
    if (widget && widget->isPluginView())
        return toPluginView(widget)->hasContentsLayer();
    return false;
}

IntRect LayoutEmbeddedObject::contentsRect() const
{
    const LayoutSize* overriddenIntrinsicSize = nullptr;
    LayoutSize size;
    Widget* widget = this->widget();
    if (widget && widget->isPluginView()) {
        IntSize isize = toPluginView(widget)->contentsLayerIntrinsicSize();
        if (!isize.isEmpty()) {
            size = LayoutSize(isize);
            overriddenIntrinsicSize = &size;
        }
    }
