...
void DOMWindow::debug(const String& string) const {
#if defined(HBBTV_ON)
    if (document())
        document()->addConsoleMessage(ConsoleMessage::create(JSMessageSource, DebugMessageLevel, "[DOMDEBUG] " + string));
#else
    // This api is hbbtv feature implemented only if HBBTV_ON is defined.
#endif
}
