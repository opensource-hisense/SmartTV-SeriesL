...
HTMLPlugInElement::HTMLPlugInElement(const QualifiedName& tagName, Document& doc, bool createdByParser, PreferPlugInsForImagesOption preferPlugInsForImagesOption)
    ...
    , m_needsWidgetUpdateBecauseOfPluginRedetermination(false)
#endif
...
#if defined(HBBTV_ON)
    // FIXME: this logic needs fix once Google fix this problem in M49 or later.
    if (plugin && context.performingReattach && !needsWidgetUpdateBecauseOfPluginRedetermination()) {
...
#if defined(HBBTV_ON)
        {
            WTFLogAlways("ZZZZ: HTMLPlugInElement::loadPlugin: widget=%p\n",
                        widget.get());
        }
#endif
...
bool HTMLPlugInElement::shouldUsePlugin(const KURL& url, const String& mimeType, bool hasFallback, bool& useFallback)
{
#if defined(HBBTV_ON)
    {
        ...
        if (f_document_is_hbbtv_document && (mimeType == "video/mp4" || mimeType == "audio/mp4" || mimeType == "audio/mpeg")) {
            // override with our strange plugin.
            useFallback = false;
            return true;
        }
    }
