...
void HTMLObjectElement::reloadPluginOnAttributeChange(const QualifiedName& name)
{
    ...
    setNeedsWidgetUpdate(true);
#if defined(HBBTV_ON)
    // FIXME: this logic needs fix once Google fix this problem in M49 or later.
    // the above setNeedsWidgetUpdate(true) flag is not really useful, because of
    // recalcStyle() -> recalcOwnStyle() (|localChange| == Reattach) -> reattach(),
    // and HTMLPlugInElement::detach() does setNeedsWidgetUpdate(true) even if this
    // widget does not really needs update.
    // so we added another flag to see if this widget really needs change.
    if (needsInvalidation) {
        setNeedsWidgetUpdateBecauseOfPluginRedetermination(true);
        lazyReattachIfNeeded();
        setNeedsWidgetUpdateBecauseOfPluginRedetermination(false);
    }
#else
    if (needsInvalidation)
        lazyReattachIfNeeded();
