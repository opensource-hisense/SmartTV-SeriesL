...
Document::Document(const DocumentInit& initializer, DocumentClassFlags documentClasses)
    ...
    , m_wellFormed(false)
#if defined(HBBTV_ON)
    , m_metaElementHasHbbTvMimeType(false)
#endif
    , m_printing(false)
    , m_wasPrinting(false)
...
#if defined(HBBTV_ON)
        if (isXMLAITDocument())
            return "application/vnd.dvb.ait+xml";
#endif
        if (isXHTMLDocument())
...
#if defined(HBBTV_ON)
        if (isXMLAITDocument())
            return XMLDocument::createXMLAIT(init.withRegistrationContext(registrationContext()));
#endif
        if (isXHTMLDocument())
...
void Document::finishedParsing()
{
    ...
    for (HTMLMetaElement* metaElement = head() ? Traversal<HTMLMetaElement>::firstChild(*head()) : 0; metaElement; metaElement = Traversal<HTMLMetaElement>::nextSibling(*metaElement)) {
        if (equalIgnoringCase(metaElement->httpEquiv(), "content-type")) {
            AtomicString mime_type = extractMIMETypeFromMediaType(metaElement->content());
            // an extra kludge for itv.mit-xperts.com whose mime type is prefixed with "Content-Type:".
            if (mime_type.impl()->startsWithIgnoringASCIICase("content-type:", 13))
                mime_type = AtomicString(mime_type.impl(), 13, mime_type.length() - 13);
            if (!mime_type.isEmpty()) {
                if (equalIgnoringCase(mime_type, "application/ce-html+xml") || equalIgnoringCase(mime_type, "application/vnd.hbbtv.xhtml+xml")) {
                    m_metaElementHasHbbTvMimeType = true;
                    break;
                }
            }
        }
