...
#if defined(HBBTV_ON)
    unsigned short aitAppID() const { return m_aitAppID; }
    void setAITAppID(const unsigned short appID) { m_aitAppID = appID; }

    unsigned int aitOrgID()  const { return m_aitOrgID; }
    void setAITOrgID(const unsigned int orgID) { m_aitOrgID = orgID; }

    String aitControlCode() const { return m_aitControlCode; }
    void setAITControlCode(const String& controlCode) { m_aitControlCode = controlCode; }

    String aitVisibility() const { return m_aitVisibility; }
    void setAITVisibility(const String& aitvisibility) { m_aitVisibility = aitvisibility; }

    bool aitServiceBound() const { return m_aitServiceBound; }
    void setAITServiceBound(const bool serviceBound) { m_aitServiceBound = serviceBound; }

    unsigned char aitPriority() const { return m_aitPriority; }
    void setAITPriority(const unsigned char priority) { m_aitPriority = priority; }

    String aitUrlBase() const { return m_aitUrlBase; }
    void setAITUrlBase(const String& urlBase) { m_aitUrlBase = urlBase; }

    String aitApplicationLocation() const { return m_aitApplicationLocation; }
    void setAITApplicationLocation(const String& appLocation) { m_aitApplicationLocation = appLocation; }
#endif //HBBTV_ON
...
#if defined(HBBTV_ON)
    bool isHbbTvDocumentAccordingToMetaElement() const { return m_metaElementHasHbbTvMimeType; }
    bool isXMLAITDocument() const { return m_documentClasses & XMLAITDocumentClass; }
    void clearXMLAITDocument() { m_documentClasses &= ~XMLAITDocumentClass; }
#endif
...
private:
...
#if defined(HBBTV_ON)
    // true if the head element has a meta element of which "http-equiv"
    // attribute is "content-type" and "content" attribute has a mime type
    // for HbbTv document i.e. CE-HTML or vnd.hbbtv prefixed XHTML.
    // N.b. even if the application modifies meta tags later, this value
    // will not change. This is the same as isHTMLDocument(),
    // isCEHTMLDocument(), and such; if the application later changes the
    // content type via Document::setMimeType(), the value returned from
    // Document::contentType() will change, but the value returned from
    // isHTMLDocument(), isCEHTMLDocument(), and such will not change.
    bool m_metaElementHasHbbTvMimeType;
#endif
    ...
    String m_aitControlCode;
    String m_aitVisibility;
    bool m_aitServiceBound;
    unsigned char m_aitPriority;
    String m_aitUrlBase;
    String m_aitApplicationLocation;
#endif
