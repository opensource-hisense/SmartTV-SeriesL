...
#if defined(HBBTV_ON)
double Element::contentWidth()
{
    document().updateLayoutIgnorePendingStylesheets();

    if (LayoutBox* layoutObject = layoutBox())
        return adjustLayoutUnitForAbsoluteZoom(layoutObject->contentWidth(), *layoutObject).toDouble();
    return 0.f;
}

double Element::contentHeight()
{
    document().updateLayoutIgnorePendingStylesheets();

    if (LayoutBox* layoutObject = layoutBox())
        return adjustLayoutUnitForAbsoluteZoom(layoutObject->contentHeight(), *layoutObject).toDouble();
    return 0.f;
}
#endif
