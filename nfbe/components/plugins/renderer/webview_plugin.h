 ...
 public:
  class Delegate {
   public:
...
#if defined(HBBTV_ON)
    // for oipf video
    // we need a method which is not const qualified.
    virtual v8::Local<v8::Object> GetV8ScriptableObject_1(v8::Isolate*);

    // for oipf video
    // Called when the size of the plugin is updated.
    virtual void OnResize(const blink::WebSize size) {}
#endif
    ...
    virtual blink::WebSize contentsLayerIntrinsicSize() const;
    virtual bool shouldReportDetailedMessageForSourceWvD(
        const blink::WebString& source);
    virtual void DetailedConsoleMessageAddedWvD(
  ...
  blink::WebMediaPlayer* createMediaPlayer(
      ...
      blink::WebMediaSession*) override;
