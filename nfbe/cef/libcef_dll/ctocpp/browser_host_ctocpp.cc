...
CefHBBTVBrowserPtr CefBrowserHost::CreateNewBrowserWindow(
  ...
  // Execute
  cef_hbbtvbrowser_ptr_t _retval = cef_browser_host_create_new_browser_window(
      url.GetStruct());
...
CefHBBTVWebContentsPtr CefBrowserHostCToCpp::GetWebContents() {
  cef_browser_host_t* _struct = GetStruct();
  if (CEF_MEMBER_MISSING(_struct, get_web_contents))
    return NULL;

  // AUTO-GENERATED CONTENT - DELETE THIS COMMENT BEFORE MODIFYING

  // Execute
  cef_hbbtvweb_contents_ptr_t _retval = _struct->get_web_contents(_struct);

  // Return type: simple
  return _retval;
}

void CefBrowserHostCToCpp::LoadURLNoFocus(const CefString& url,
    const bool focused) {
  cef_browser_host_t* _struct = GetStruct();
  if (CEF_MEMBER_MISSING(_struct, load_urlno_focus))
    return;
  ...
  // Execute
  _struct->load_urlno_focus(_struct,
      url.GetStruct(),
      focused);
}
