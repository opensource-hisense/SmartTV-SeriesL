...
def process_cmake_template_segment_acs(segment, segment_ct, cmake_path, variables):
    result = ''
    if variables['acs_use_hbbtv']: # |variables| is tampred to pass our options.
        values = eval('{' + segment + '}', {'__builtins__': None}, None)
        if 'use_hbbtv' in values:
            if values['use_hbbtv'] == '1':
                if 'code' in values:
                    result += values['code']
    if variables['acs_use_shared_libs']: # |variables| is tampred to pass our options.
        values = eval('{' + segment + '}', {'__builtins__': None}, None)
        if 'use_shared_libs' in values:
            if values['use_shared_libs'] == '1':
                if 'code' in values:
                    result += values['code']
    # do not do strip() to preserve indent.
    return result;
...
def process_cmake_template(input, output, variables, quiet = False):
    ...
    segment_ct = 0
    while True:
        # access:_S process whichever comes first.
        # n.b. |variables| is tampred to pass our options.
        start_cef = template.find(delim_start, end)
        start_acs = template.find(acs_delim_start, end)
        if start_acs != -1 and (start_cef == -1 or start_acs < start_cef):
            # acs comes first.
            start = start_acs
            result += template[end:start]
            end = template.find(acs_delim_end, start + len(acs_delim_start))
            if end == -1:
                break
            segment = template[start + len(acs_delim_start):end]
            segment_ct = segment_ct + 1
            result += process_cmake_template_segment_acs(segment, segment_ct, \
                                                         cmake_path, variables)
            end += len(acs_delim_end)
            continue
        # access:_E
