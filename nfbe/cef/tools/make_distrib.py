...
parser.add_option('--acs-use-hbbtv',
                  action='store_true', dest='acs_use_hbbtv', default=False,
                  help='build was configured with \'use_hbbtv=1\'')
parser.add_option('--acs-use-shared-libs',
                  action='store_true', dest='acs_use_shared_libs', default=False,
                  help='build was created with shared libraries')
...
if mode == 'standard':
  ...
  # access:_S |variables| is tampred to pass our options.
  variables['acs_use_hbbtv'] = options.acs_use_hbbtv
  variables['acs_use_shared_libs'] = options.acs_use_shared_libs
  # access:_E
...
if platform == 'windows':
  ...
  if mode == 'standard':
    ...
    if not options.allowpartial or path_exists(libcef_path):
      ...
      if options.acs_use_shared_libs:
        # shared debug libraries
        copy_file(os.path.join(build_dir, 'lib/libaccessibility.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libaura_extra.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libaura.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libbase_i18n.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libbase.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libblink_common.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libblink_platform.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libblink_web.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libboringssl.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libcc_blink.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libcc_proto.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libcc.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libcc_surfaces.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libaura_extra.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libaura.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libbase_i18n.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libbase.so'), dst_dir, options.quiet)
        ...
        copy_file(os.path.join(build_dir, 'lib/libcc.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libcc_surfaces.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libchromium_sqlite3.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libcompositor.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libcontent.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libcrcrypto.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libdbus.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libdevice_battery.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libdevice_bluetooth.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libdevice_core.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libdevice_event_log_component.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libdevice_vibration.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libdisplay.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libdisplay_types.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libdisplay_util.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libevents_base.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libevents_devices.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libevents_ipc.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libevents_ozone_evdev.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libevents_ozone_layout.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libevents_ozone.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libevents_platform.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libevents.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libffmpeg.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libfontconfig.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libgesture_detection.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libgfx_geometry.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libgfx_ipc_skia.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libgfx_ipc.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libgfx.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libgin.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libgles2_c_lib.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libgles2_implementation.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libgles2_utils.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libgl_in_process_context.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libgl_wrapper.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libgpu_blink.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libgpu.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libhunspell.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libicui18n.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libicuuc.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libipc_mojo.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libipc.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libkeyed_service_content.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libkeyed_service_core.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libmedia_blink.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libmedia.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libmidi.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libmodules.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libmojo_common_lib.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libmojo_geometry_lib.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libmojo_message_pump_lib.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libmojo_system_impl.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libnative_theme.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libnet.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libnet_with_v8.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libonc_component.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libozone_base.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libozone.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libplatform_handle.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libppapi_host.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libppapi_proxy.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libppapi_shared.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libprefs.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libprinting.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libprotobuf_lite.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libproxy_config.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libsandbox_services.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libscheduler.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libseccomp_bpf_helpers.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libseccomp_bpf.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libsessions_content.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libshared_memory_support.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libshell_dialogs.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libskia.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libsnapshot.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libsql.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libstorage_common.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libstorage.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libsuid_sandbox_client.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libsurface.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libsync_core.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libsync_proto.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libtracing.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libtranslator.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libui_base_ime.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libui_base.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libui_data_pack.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libui_touch_selection.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/liburl_ipc.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/liburl_lib.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/liburl_matcher.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libusb.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libuser_prefs.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libv8.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libwebcore_shared.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libwm.so'), dst_dir, options.quiet)
        copy_file(os.path.join(build_dir, 'lib/libwtf.so'), dst_dir, options.quiet)
    else:
      sys.stderr.write("No Debug build files.\n")
  ...
  if options.acs_use_hbbtv:
    copy_dir(os.path.join(cef_dir, '../access/peer'), os.path.join(output_dir, 'libhbbtv_peer/access/peer'), options.quiet)
    process_cmake_template(os.path.join(cef_dir, 'libhbbtv_peer', 'CMakeLists.txt.in'), \
                           os.path.join(output_dir, 'libhbbtv_peer', 'CMakeLists.txt'), \
                           variables, options.quiet)
