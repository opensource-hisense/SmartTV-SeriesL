...
#if defined(NFBE_DTV_SOCIO) // TODO This fixes might be provisinal. See #104144.
...
// FIXME: move the resources out of shell. */
...
void PlatformPollFreemem(void) {
  // C++ istream doesn't have scanf() like function...
  ...
  int critical, moderate;
  if (command_line->HasSwitch("memory-pressure-thresholds-used-memory-mb")) {
    std::string thresholds_str = command_line->GetSwitchValueASCII("memory-pressure-thresholds-used-memory-mb");
    sscanf(thresholds_str.c_str(), "%d,%d", &moderate, &critical);
  } else {
