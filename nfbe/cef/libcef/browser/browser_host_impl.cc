...
#include "content/shell/browser/acs/acs_web_contents_observer.h"
...
#if defined(HBBTV_ON)
// This will be the browser helper owned here but used by the HBBTV App Manager
// whenever it is required to create a new browser window.
static scoped_ptr<CreateBrowserHelper> hbbtv_apps_helper = nullptr;
#endif
...
#else
  CHECK(client);
  // The App Manager must be already created in the client handler.
  content::hbbtv::HBBTVAppMgr* hbbtv_app_mgr =
          (content::hbbtv::HBBTVAppMgr*)client->GetHBBTVAppMgr();
  if (!hbbtv_app_mgr) {
      LOG(ERROR) << "The ClientHandler must initialize "
                          "the unique HBBTV Application Manager";
      return nullptr;
  }

  // This will be used automatically by all the windows spawned by HBBTVAppMgr.
  CreateBrowserHelper* helper =
      new CreateBrowserHelper(windowInfo, client, url, settings,
                              request_context);
  hbbtv_apps_helper.reset(helper);

  // The App Manager calls CefBrowserHostImpl::CreateNewBrowserWindow()
  // during the call to LoadApp().
  hbbtv_app_mgr->LoadApp(url);

  // The return value is discarded by the caller because the whole creation
  // process is asynchronous therefore we just return NULL here.
...
#if defined(HBBTV_ON)
// static
CefHBBTVBrowserPtr CefBrowserHost::CreateNewBrowserWindow(
    const CefString& url) {
  // We are reusing the parameters set when we created the first window.
  CHECK(hbbtv_apps_helper);
  CefBrowserHostImpl::CreateParams create_params;
  create_params.window_info.reset(
      new CefWindowInfo(hbbtv_apps_helper->window_info_));
  create_params.client = hbbtv_apps_helper->client_;
  create_params.url = url;
  create_params.settings = hbbtv_apps_helper->settings_;
  create_params.request_context = hbbtv_apps_helper->request_context_;
...
void CefBrowserHostImpl::LoadURLNoFocus(const CefString& url,
                                        const bool focused) {
  LOG(INFO) << "CefBrowserHostImpl::LoadURL Loading URL " << url
            << "with focus " << focused;
  if (focused) {
    LoadURL(CefFrameHostImpl::kMainFrameId,
            url, content::Referrer(),
            ui::PAGE_TRANSITION_TYPED, std::string());
  } else {
    LoadURLSkipFocus(CefFrameHostImpl::kMainFrameId,
                     url, content::Referrer(),
                     ui::PAGE_TRANSITION_TYPED, std::string());
  }
}
...
// This is a perfect copy of LoadURL only without calling OnSetFocus
void CefBrowserHostImpl::LoadURLSkipFocus(
    int64 frame_id,
    const std::string& url,
    const content::Referrer& referrer,
    ui::PageTransition transition,
...
CefBrowserHostImpl::CefBrowserHostImpl(
...
#if defined(HBBTV_ON)
  web_contents->SetHbbTVDelegate((content::hbbtv::HBBTVAppMgr*)client_->GetHBBTVAppMgr());
#endif
  ...
  // Start the autotest and statistics observer.
  content::AcsWebContentsObserver::CreateForWebContents(web_contents);
}
