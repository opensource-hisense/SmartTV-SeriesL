...
void CefBrowserMainParts::PreMainMessageLoopRun() {
  ...
  if (!command_line->HasSwitch(switches::kProcessType)) {
    if (command_line->HasSwitch(switches::kPerformanceMonitorLight)) {
      performance_monitor_light::
          PerformanceMonitorLight::
              GetInstance()->Initialize();
    } else if (command_line->HasSwitch(switches::kAcsAutoTest)) {
      // initialize  the performance monitoring every 3s
      // if the interval between two autotest url loading is 10s
      // this should be enough for autotest
      // in addition don't dump information about each process to console
      performance_monitor_light::
          PerformanceMonitorLight::
              GetInstance()->Initialize(false, 3);
    }
  }
