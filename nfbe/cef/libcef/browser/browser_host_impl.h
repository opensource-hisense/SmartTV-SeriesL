...
class CefBrowserHostImpl : public CefBrowserHost,
...
#if defined(HBBTV_ON)
  // Returns the HBBTV Application manager associated with this browser.
  content::hbbtv::HBBTVAppMgr* GetHBBTVAppMgr();
#endif

  // Load the given URL in the main frame of the current browser
  // and focus it or not depending on the focus parameter.
  void LoadURLNoFocus(const CefString& url, const bool focused) override;

  // WebContents accessor.
  CefHBBTVWebContentsPtr GetWebContents() {return web_contents_.get();};
 ...
 private:
  ...
  // Load the specified URL without calling OnSetFocus()
  void LoadURLSkipFocus(int64 frame_id,
               const std::string& url,
               const content::Referrer& referrer,
               ui::PageTransition transition,
               const std::string& extra_headers);
