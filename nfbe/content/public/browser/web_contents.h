...
#if defined(HBBTV_ON)
// FIXME: We shouldn't use implementation class.
class WebContentsImpl;

class WebContentsHbbTVDelegate {
 public:
  virtual ~WebContentsHbbTVDelegate() {};
  virtual void WillRemoveWebContents(WebContentsImpl*) = 0;
  virtual bool OnHbbTVMessageReceived(const IPC::Message& message,
                                      WebContentsImpl* receiver_wci,
                                      RenderFrameHost* receiver_rfh) = 0;
  // Using void* to avoid dependency to frame_messages.h
  // which has definiton of FrameHostMsg_DidFinishLoadXMLAIT_Params
  virtual void DidFinishLoadXMLAIT(const void* params) = 0;
  virtual void DidRemoveRenderFrame(WebContentsImpl*, int routing_id) = 0; // RenderFrameRemoved
};
#endif
...
#if defined(HBBTV_ON)
  virtual void SetHbbTVDelegate(WebContentsHbbTVDelegate*) = 0;
#endif
