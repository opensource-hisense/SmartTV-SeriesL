...
class RendererBlinkPlatformImpl::MimeRegistry
 ...
 public:
  ...
  bool supportsMediaSourceMIMEType(const blink::WebString& mime_type,
      ...
      unsigned bitrate, unsigned channels,
      const blink::WebString& eotf) override;
#else
  bool supportsMediaSourceMIMEType(const blink::WebString& mime_type,
                                   const blink::WebString& codecs) override;
#endif
...
bool RendererBlinkPlatformImpl::MimeRegistry::supportsMediaSourceMIMEType(
    ...
    const WebString& codecs,
    unsigned width, unsigned height, unsigned framerate,
    unsigned bitrate, unsigned channels, const blink::WebString& eotf) {
#else
bool RendererBlinkPlatformImpl::MimeRegistry::supportsMediaSourceMIMEType(
    const blink::WebString& mime_type,
    const WebString& codecs) {
  ...
  return media::StreamParserFactory::IsTypeSupported(
      mime_type_ascii, parsed_codec_ids, width, height, framerate, bitrate,
      channels, eotf.utf8());
#else
  return media::StreamParserFactory::IsTypeSupported(
