...
void RenderView::ApplyWebPreferences(const WebPreferences& prefs,
                                     WebView* web_view) {
#if defined(OS_ANDROID)
  bool acs_enable_automatic_scaling_value = true;
  {
    const base::CommandLine& command_line =
        *base::CommandLine::ForCurrentProcess();
    if (command_line.HasSwitch(switches::kAcsEnableAutomaticScaling)) {
      std::string value = command_line.GetSwitchValueASCII(
          switches::kAcsEnableAutomaticScaling);
      if (base::LowerCaseEqualsASCII(value, "disable"))
        acs_enable_automatic_scaling_value = false;
    }
  }
#endif
...
#if defined(OS_ANDROID)
  {
    float mins = prefs.default_minimum_page_scale_factor;
    float maxs = prefs.default_maximum_page_scale_factor;
    if (!acs_enable_automatic_scaling_value) {
      mins = 1;
      maxs = 1;
    }
    web_view->setDefaultPageScaleLimits(mins, maxs);
  }
#else
  ...
  if (!acs_enable_automatic_scaling_value) {
    ...
    settings->setUseMobileViewportStyle(false);
  }
#endif
