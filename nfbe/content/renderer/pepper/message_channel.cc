...
#if defined(HBBTV_ON)
  CbipPluginObject* cbip_plugin_object = GetCbipPluginObject(isolate);
  if (cbip_plugin_object) {
    DCHECK( !GetPluginObject(isolate) );
    gin::NamedPropertyInterceptor* g = cbip_plugin_object->asGinNamedPropertyInterceptor();
    if (g) {
      return g->GetNamedProperty(isolate, identifier);
    }
  }
#endif
...
bool MessageChannel::SetNamedProperty(v8::Isolate* isolate,
  ...
  if (cbip_plugin_object) {
    ...
    gin::NamedPropertyInterceptor* g = cbip_plugin_object->asGinNamedPropertyInterceptor();
    if (g) {
      return g->SetNamedProperty(isolate, identifier, value);
    }
  }
#endif
...
#if defined(HBBTV_ON)
CbipPluginObject* MessageChannel::GetCbipPluginObject(v8::Isolate* isolate) {
  return CbipPluginObject::FromV8Object(isolate,
      v8::Local<v8::Object>::New(isolate, passthrough_object_));
}
#endif
