...
#if defined(HBBTV_ON)
class PepperPluginInstanceImplDelegate {
 public:
  virtual void DidChangeView(const gfx::Rect& rect) = 0;
};
#endif
...
#if defined(HBBTV_ON)
  void Initialize2(const std::string& mime_type);
#endif
...
#if defined(HBBTV_ON)
  // Tracks all live CbipPluginObjects.
  void AddCbipPluginObject(CbipPluginObject* cbip_plugin_object);
  void RemoveCbipPluginObject(CbipPluginObject* cbip_plugin_object);
#endif
...
#if defined(HBBTV_ON)
  void SetDelegate(PepperPluginInstanceImplDelegate* delegate) { delegate_ = delegate; };
#endif
 private:
...
#if defined(HBBTV_ON)
  typedef std::set<CbipPluginObject*> CbipPluginObjectSet;
  CbipPluginObjectSet live_cbip_plugin_objects_;
#endif
  ...
  PepperPluginInstanceImplDelegate* delegate_;
#endif
