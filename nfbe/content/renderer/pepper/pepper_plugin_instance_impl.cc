...
#if !defined(ANDROID)
// Disabled this in ASE as it is most probably not used.
// As of 51.0.2704.29 the Android build is failing on this line
// because the bit_cast complains that ppapi::WebKitGamepads is not
// trivially copyable.
// What is also weird is that the gcc static_assert at line 73 in bit_cast.h
// which is for GCC 5.1, but the Android arm compiler is GCC 4.9.
// Another strange thing is that OS_ANDROID is not defined here!
// Nevertheless this encapsulation should disappear if rebasing to a newer
// version which is building properly under Android
...
#if defined(HBBTV_ON)
  CbipPluginObjectSet cbip_plugin_object_copy;
  live_cbip_plugin_objects_.swap(cbip_plugin_object_copy);
  for (CbipPluginObjectSet::iterator i = cbip_plugin_object_copy.begin();
       i != cbip_plugin_object_copy.end();
       ++i) {
    (*i)->InstanceDeleted();
  }
#endif
...
#if defined(HBBTV_ON)
void PepperPluginInstanceImpl::Initialize2(const std::string& mime_type) {
...
#if defined(HBBTV_ON)
  if (module_->path().AsUTF8Unsafe() == cbip::kInternalCbipPluginFileName)
    return CbipPluginObject::Create(this, mime_type_);
#endif
...
#if defined(HBBTV_ON)
  if (delegate_) {
    gfx::Rect rect(view_data.rect.point.x, view_data.rect.point.y, view_data.rect.size.width, view_data.rect.size.height);
    delegate_->DidChangeView(rect);
  }
#endif
...
#if defined(HBBTV_ON)
void PepperPluginInstanceImpl::AddCbipPluginObject(CbipPluginObject* cbip_plugin_object) {
  DCHECK(live_cbip_plugin_objects_.find(cbip_plugin_object) ==
         live_cbip_plugin_objects_.end());
  live_cbip_plugin_objects_.insert(cbip_plugin_object);
}

void PepperPluginInstanceImpl::RemoveCbipPluginObject(CbipPluginObject* cbip_plugin_object) {
  // Don't actually verify that the object is in the set since during module
  // deletion we'll be in the process of freeing them.
  live_cbip_plugin_objects_.erase(cbip_plugin_object);
}
#endif
