...
#if defined(HBBTV_ON)
IPC_STRUCT_BEGIN(FrameHostMsg_DidFinishLoadXMLAIT_Params)
  // application Identifier: appID - unsigned short
  IPC_STRUCT_MEMBER(int, appID)

  // application Identifier: orgID - unsigned int
  IPC_STRUCT_MEMBER(int, orgID)

  // AIT Control Code string as parsed from the XML AIT document
  IPC_STRUCT_MEMBER(std::string, controlCode)

  // AIT Visibility string as parsed from the XML
  IPC_STRUCT_MEMBER(std::string, visibility)

  // Service bound flag
  IPC_STRUCT_MEMBER(bool, serviceBound)

  // application priority- unsigned char
  IPC_STRUCT_MEMBER(int, priority)

  // AIT base URL as parsed from the XML and stored in a GURL
  IPC_STRUCT_MEMBER(GURL, urlBase)

  // application location string as parsed from the XML
  IPC_STRUCT_MEMBER(std::string, applicationLocation)
IPC_STRUCT_END()
#endif //HBBTV_ON
...
#if defined(HBBTV_ON)
// Sent when a renderer is done loading and parsing an XML AIT.
// This message will have a lot of parameters packed into the Params structure.
IPC_MESSAGE_ROUTED1(FrameHostMsg_DidFinishLoadXMLAIT,
                    FrameHostMsg_DidFinishLoadXMLAIT_Params)
#endif
