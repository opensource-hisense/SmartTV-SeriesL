...
#if defined(HBBTV_ON)
  if (hbbtv_delegate_)
    hbbtv_delegate_->WillRemoveWebContents(this);
#endif
...
bool WebContentsImpl::OnMessageReceived(RenderViewHost* render_view_host,
  ...
  if (IPC_MESSAGE_CLASS(message) == HbbtvMsgStart) {
    ...
    if (!hbbtv_delegate_)
      return false;
    return hbbtv_delegate_->OnHbbTVMessageReceived(message, this,
                                                   render_frame_host);
  }

  if (IPC_MESSAGE_CLASS(message) == OipfMsgStart) {
    OIPFMessageHandler* oipf_msg_handler =
                                  OIPFMessageHandler::GetOIPFMessageHandler();
    if (!oipf_msg_handler)
      return false;
    return oipf_msg_handler->OnOIPFMessageReceived(message, this,
                                                   render_frame_host);
  }
#endif
...
#if defined(HBBTV_ON)
    IPC_MESSAGE_HANDLER(FrameHostMsg_DidFinishLoadXMLAIT, OnDidFinishLoadXMLAIT)
#endif
...
#if defined(HBBTV_ON)
void WebContentsImpl::SetHbbTVDelegate(WebContentsHbbTVDelegate* delegate) {
  hbbtv_delegate_ = delegate;
}
#endif
...
#if defined(HBBTV_ON)
void WebContentsImpl::OnDidFinishLoadXMLAIT(const FrameHostMsg_DidFinishLoadXMLAIT_Params& params) {
  if (hbbtv_delegate_)
    hbbtv_delegate_->DidFinishLoadXMLAIT((void*)&params);
}
#endif
...
#if defined(HBBTV_ON)
  // because we do not want to modify all those shell implementations....
  // FIXME: check: what is the difference
  // between RenderFrameDeleted and FrameDeleted ? ? ?
  if (hbbtv_delegate_) {
    hbbtv_delegate_->DidRemoveRenderFrame(this,
