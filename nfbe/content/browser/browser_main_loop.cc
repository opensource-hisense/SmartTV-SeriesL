...
#if defined(USE_X11)
#if defined(HBBTV_ON)  // FIXME: HBBTV_ON is not an appropriate macro.
...
#if defined(HBBTV_ON)
  {
    cbip_pipe_watcher_.reset(new CbipPipeWatcher(CbipInternals::GetInstance()));
  }
#endif
...
#if defined(HBBTV_ON)  // FIXME: HBBTV_ON is not an appropriate macro.
  // FIXME: this block should be enabled if CEF patch is applied,
  // but currently there is no macro which tells it that is.
  // content_shell build with use_hbbtv=0 still dies.
