...
void RenderProcessHostImpl::CreateMessageFilters() {
...
#if defined(HBBTV_ON)
  CbipInternals* cbip_internals = CbipInternals::GetInstance();
#endif
  ...
  /*
   * FIXME(ryota): check.
   * some needs to install its service or context or manager to the
   * render_message_filter, which handles ChildProcessMsgStart,
   * FrameMsgStart, ViewMsgStart.  why ? ? ?
   */
...
#if defined(HBBTV_ON)
  AddFilter(new CbipJs00MessageFilter(GetID(), cbip_internals->task_runner()));
#endif
}
