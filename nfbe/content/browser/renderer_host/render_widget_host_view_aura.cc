...
// define this if you want this class to handle the F1 and F12 keys
// FIXME: this should be deprecated and moved to the application layers
#define ACS_HANDLE_KEYS 1
...
void RenderWidgetHostViewAura::OnKeyEvent(ui::KeyEvent* event) {
  ...
  if (is_fullscreen_ && event->key_code() == ui::VKEY_ESCAPE) {
    ...
    if (event->key_code() == ui::VKEY_F1) {
        LOG(INFO) << std::endl << "EXIT Key Pressed ! Browser Process will be shut down. GPU Process will die..." << std::endl;
        Shutdown();
        base::MessageLoop::current()->PostDelayedTask(FROM_HERE,
                                               base::MessageLoop::QuitWhenIdleClosure(), base::TimeDelta::FromMilliseconds(50));
        event->SetHandled();
        return;
    }
    if (event->key_code() == ui::VKEY_F12) {
       if (event->type() == ui::ET_KEY_RELEASED) {
           ...
           // parse the command line and extract the first valid URL
           const base::CommandLine::StringVector& params = base::CommandLine::ForCurrentProcess()->GetArgs();
           scoped_ptr<GURL> url = NULL;
           for (size_t i = 0; i < params.size(); ++i) {
                   GURL u(params[i]);
               if (u.is_valid()) {
                     ChildProcessSecurityPolicy* policy =
                             ChildProcessSecurityPolicy::GetInstance();
                     if (policy->IsWebSafeScheme(u.scheme()) ||
                             u.SchemeIs(url::kFileScheme) ||
                             (u.spec().compare(url::kAboutBlankURL) == 0)) {
                                     url.reset(new GURL(u));
                                     break;
                     }
                   }
           }
           ...
           if (!url)
               url.reset(new GURL("http://www.access-company.com"));

           host_->GetProcess()->FilterURL(false, url.get());
           LOG(INFO) << std::endl << " Loading startup page on key request : " << url->GetContent() << std::endl;

           ui::PageTransition transition = ui::PAGE_TRANSITION_AUTO_TOPLEVEL;
           int frame_tree_node_id = rfhi->frame_tree_node()->frame_tree_node_id();

           OpenURLParams ou_params(
                        *url, Referrer(),
                        frame_tree_node_id, CURRENT_TAB, transition,
                        true /* is_renderer_initiated */);
           web_contents->OpenURL(ou_params);

       }
       event->SetHandled();
       return;
    }
#endif //ACS_HANDLE_KEYS
