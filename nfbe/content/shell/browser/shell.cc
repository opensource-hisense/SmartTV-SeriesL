...
#if defined(HBBTV_ON)
  web_contents->SetHbbTVDelegate(
      ShellContentBrowserClient::Get()->shell_browser_main_parts()->hbbtv_app_mgr());
#endif
...
Shell* Shell::CreateNewWindow(BrowserContext* browser_context,
  ...
  AcsWebContentsObserver::CreateForWebContents(web_contents);
  if (!url.is_empty())
    shell->LoadURL(url);
  return shell;
}
...
#if defined(HBBTV_ON)
void Shell::LoadURLNoFocus(const GURL& url) {
...
#if defined(HBBTV_ON)
bool Shell::PreHandleKeyboardEvent(WebContents* source,
                                    const NativeWebKeyboardEvent& event,
                                    bool* is_keyboard_shortcut) {
  hbbtv::HBBTVAppMgr* mgr =
      ShellContentBrowserClient::Get()->shell_browser_main_parts()->hbbtv_app_mgr();
  // passing over the event to the hbbtv application manager
  if (mgr)
    return mgr->PreHandleKeyboardEvent(source, event);
  return false;
}
