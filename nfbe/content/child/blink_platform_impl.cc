...
#if defined(HBBTV_ON)
bool BlinkPlatformImpl::hbbtvRemapKeyboardEventValuesFromDomKey(
    int dom_key, int& windows_keycode, const char*& key_identifier) {
#if defined(OS_ANDROID)
  // Windows has play button and pause button, but those are not meant
  // for media control.  Windows has a media play/pause button instead.
  // Android defines all of them.
  if (dom_key == ui::DomKey::MEDIA_PLAY) {
    DCHECK(windows_keycode == ui::VKEY_MEDIA_PLAY_PAUSE);
    DCHECK(!::strcmp(key_identifier, "MediaPlayPause"));
    windows_keycode = ui::CEA_VK_PLAY;
    key_identifier = "MediaPlay";
    return true;
  }
  if (dom_key == ui::DomKey::MEDIA_PAUSE) {
    DCHECK(windows_keycode == ui::VKEY_MEDIA_PLAY_PAUSE);
    DCHECK(!::strcmp(key_identifier, "MediaPlayPause"));
    windows_keycode = ui::VKEY_PAUSE;
    key_identifier = "Pause";
    return true;
  }
  if (dom_key == ui::DomKey::MEDIA_PLAY_PAUSE) {
    DCHECK(windows_keycode == ui::VKEY_MEDIA_PLAY_PAUSE);
    DCHECK(!::strcmp(key_identifier, "MediaPlayPause"));
    // CEA-2014-A Annex F does not define VK_PLAY_PAUSE.
    // ETSI TS 102 796 V1.2.1 (2012-11) defines VK_PLAY_PAUSE but does not
    // define its numeric value.
    // the below value, 463, is defined by us.
    // certain site (e.g. http://itv.ard.de/ardstart/index.php) assumes that
    // the platform only has VK_PLAY and VK_PAUSE, and so does not handle
    // VK_PLAY_PAUSE.  certain site
    // (e.g. http://hbbtv.ardmediathek.de/hbbtv-ard/mediathek/index.html)
    // thinks that the numeric value of VK_PLAY_PAUSE is 407, which
    // CEA-2014-A defines as the value of VK_GREY.
    windows_keycode = ui::CEA_VK_PLAY_PAUSE;
    return true;
  }
  if (dom_key == ui::DomKey::MEDIA_STOP) {
    DCHECK(windows_keycode == ui::VKEY_MEDIA_STOP);
    DCHECK(!::strcmp(key_identifier, "MediaStop"));
    windows_keycode = ui::CEA_VK_STOP;
    return true;
  }
  if (dom_key == ui::DomKey::MEDIA_FAST_FORWARD) {
    DCHECK(windows_keycode == ui::VKEY_OEM_104);
    DCHECK(!::strcmp(key_identifier, "U+00E4"));
    windows_keycode = ui::CEA_VK_FAST_FWD;
    key_identifier = "FastFwd";
    return true;
  }
  if (dom_key == ui::DomKey::MEDIA_REWIND) {
    DCHECK(windows_keycode == ui::VKEY_OEM_103);
    DCHECK(!::strcmp(key_identifier, "U+00E3"));
    windows_keycode = ui::CEA_VK_REWIND;
    key_identifier = "MediaRewind";
    return true;
  }
  if (dom_key == ui::DomKey::CEA_BACK) {
    // the third back button.
    DCHECK(windows_keycode == ui::CEA_VK_BACK);
    DCHECK(!::strcmp(key_identifier, "U+01CD"));
    // OIPF T1 R2 Volume5a requires this, which contradicts OIPF T1 R2 Volume5.
    // OIPF T1 R1 does not have the idea about DOM key.
    // key_identifier = "BrowserBack";
    return true;
  }
#endif // OS_ANDROID
  return false;
