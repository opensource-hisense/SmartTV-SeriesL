...
ProcessData GetProcessDataMemoryInformation(
  ...
  for (pid_t pid : pids) {
    ...
    // FIXME: This one returns always 0 because there is no previous
    // sample cpu load to compare to.
    pmi.cpu_usage = metrics->GetCPUUsage();
...
void MemoryDetails::CollectProcessData(
  ...
  current_browser.name = base::ASCIIToUTF16("NFBE");
  current_browser.process_name = base::ASCIIToUTF16("content_shell");
