...
// The 8-bit version requires UTF-8 encoding.  Use this version when you only
// need to know whether canonicalization succeeded.
URL_EXPORT bool CanonicalizeDvbHost(const char* spec,
                                    const Component& host,
                                    CanonOutput* output,
                                    Component* out_host);
URL_EXPORT bool CanonicalizeDvbHost(const base::char16* spec,
                                    const Component& host,
                                    CanonOutput* output,
                                    Component* out_host);
...
// Dvb addresses.
//
// Tries to interpret the given host name as an dvb locator or application
// locator address according to specification: ETSI TS 102 851
URL_EXPORT bool CanonicalizeDvbAddress(const char* spec, const Component& host);
URL_EXPORT bool CanonicalizeDvbAddress(const base::char16* spec,
                                       const Component& host);
...
// Use for dvb URLs.
URL_EXPORT bool CanonicalizeDvbURL(const char* spec,
                                   int spec_len,
                                   const Parsed& parsed,
                                   CharsetConverter* query_converter,
                                   CanonOutput* output,
                                   Parsed* new_parsed);
URL_EXPORT bool CanonicalizeDvbURL(const base::char16* spec,
                                   int spec_len,
                                   const Parsed& parsed,
                                   CharsetConverter* query_converter,
                                   CanonOutput* output,
