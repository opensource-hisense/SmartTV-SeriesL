...
#if defined(HBBTV_ON)
const bool kEnableDvbSpec = true;
#else
const bool kEnableDvbSpec = false;
#endif
...
#if defined(HBBTV_ON)
  {kDvbScheme, SCHEME_WITHOUT_PORT},
#endif
...
bool DoCanonicalize(const CHAR* in_spec,
  ...
  } else if (kEnableDvbSpec &&
             DoCompareSchemeComponent(spec, scheme, url::kDvbScheme)) {
    // Dvb URLs are special.
    ParseDvbURL(spec, spec_len, &parsed_input);
    success = CanonicalizeDvbURL(spec, spec_len, parsed_input,
                                 charset_converter, output, output_parsed);
