...
void DoDvbHost(const CHAR* spec,
  ...
  if (!success || !CanonicalizeDvbAddress(spec, host)) {
    // Canonicalization failed.  Set BROKEN to notify the caller.
    host_info->family = CanonHostInfo::BROKEN;
  }
...
bool CanonicalizeDvbHost(const char* spec,
                         ...
                         Component* out_host) {
  CanonHostInfo host_info;
  DoDvbHost<char, unsigned char>(spec, host, output, &host_info);
  *out_host = host_info.out_host;
  return (host_info.family != CanonHostInfo::BROKEN);
}

bool CanonicalizeDvbHost(const base::char16* spec,
                         const Component& host,
                         CanonOutput* output,
                         Component* out_host) {
  CanonHostInfo host_info;
  DoDvbHost<base::char16, base::char16>(spec, host, output, &host_info);
  *out_host = host_info.out_host;
  return (host_info.family != CanonHostInfo::BROKEN);
}
#endif
