		     ...
		     int chipsel)
{
...
#ifdef CONFIG_NAND_UNIPHIER_BBM
	int page, offs;
	int numblocks __attribute__ ((unused));
	int nrchips, ooboffs;
	size_t len = 0;
	struct nand_bbm * bbm;
	int reserveblock;
	...
	for (; chip < nrchips; chip++) {
		...
		pr_err("not reach when CONFIG_NAND_UNIPHIER_BBM is enabled!! \n");
		BUG();
#else /* CONFIG_NAND_UNIPHIER_BBM */
		/*
...
{
	struct nand_chip *psNand = mtd_to_nand(psMtd);
	u32 blockSize;
	loff_t from1;
	int status, i;
	flstate_t oldState;
...
int bbt_registerBb(struct mtd_info *psMtd, int pageNum)
{
	...
	case NAND_AREA_MIRROR:
		return -1;
	case NAND_AREA_NORMAL:
		ret = bbt_replaceBb(psMtd, pageNum);
		if(ret) {
			return -1;
		}
		break;
...
int bbt_get_bbm_info(struct mtd_info *mtd, loff_t offs, struct bbmInfo *bbm_info)
{
	struct nand_chip *psNand = mtd_to_nand(mtd);
	struct nand_bbm *psBbm;
	int chipNum;
	...
	bbm_info->btBlocks = psBbm->area[NAND_AREA_ID_BOOT].blockNum;
	bbm_info->mtBlocks = psBbm->mtBlocks;
	bbm_info->usedMtBlocks = psBbm->usedMtBlocks;
	bbm_info->altBlocks = psBbm->altBlocks;
