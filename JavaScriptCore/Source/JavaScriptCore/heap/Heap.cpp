...
// This is the maximum number of free bytes available after a garbage collect.
static size_t sMaxBytesPerCycle = 0;

#ifdef NRDP_JSC_ENABLE_GC_CALLBACKS
static Heap::GCPhaseCallback sGCPhaseCallback = 0;
# define NRDP_JSC_GC_CALLBACK(phase) if(sGCPhaseCallback) (*sGCPhaseCallback)(phase)
#else
# define NRDP_JSC_GC_CALLBACK(phase)
#endif
...
void Heap::collect(SweepToggle sweepToggle)
{
    ...
    {
        NRDP_JSC_GC_CALLBACK(GCPhase_StartMark);
        markRoots(fullGC);
        NRDP_JSC_GC_CALLBACK(GCPhase_EndMark);
    }
    ...
    if (sweepToggle == DoSweep) {
        NRDP_JSC_GC_CALLBACK(GCPhase_StartSweep);
        ...
        m_weakSet.shrink();
        m_bytesAbandoned = 0;
        NRDP_JSC_GC_CALLBACK(GCPhase_EndSweep);
    }
    ...
    if (fullGC) {
        ...
        if(sMaxBytesPerCycle)
            m_bytesAllocatedLimit = min(m_bytesAllocatedLimit, sMaxBytesPerCycle);
    }
    m_bytesAllocated = 0;
...
void Heap::setMaxBytesPerCycle(size_t maxBytesPerCycle)
{
    sMaxBytesPerCycle = maxBytesPerCycle;
}

#ifdef NRDP_JSC_ENABLE_GC_CALLBACKS
void Heap::setCallback(Heap::GCPhaseCallback callback)
{
    sGCPhaseCallback = callback;
}
#endif
