...
#define PARSE_IDENTIFIER_LCHAR(shouldCreateIdentifier)\
template <>\
template <> ALWAYS_INLINE JSTokenType Lexer<LChar>::parseIdentifier<shouldCreateIdentifier>(JSTokenData* tokenData, unsigned lexerFlags, bool strictMode)\
{\
...
PARSE_IDENTIFIER_LCHAR(true);
PARSE_IDENTIFIER_LCHAR(false);

#define PARSE_IDENTIFIER_UCHAR(shouldCreateIdentifier)\
template <>\
template <> ALWAYS_INLINE JSTokenType Lexer<UChar>::parseIdentifier<shouldCreateIdentifier>(JSTokenData* tokenData, unsigned lexerFlags, bool strictMode)\
{\
    const ptrdiff_t remaining = m_codeEnd - m_code;\
    if ((remaining >= maxTokenLength) && !(lexerFlags & LexerFlagsIgnoreReservedWords)) {\
        JSTokenType keyword = parseKeyword<shouldCreateIdentifier>(tokenData);\
...
PARSE_IDENTIFIER_UCHAR(true);
PARSE_IDENTIFIER_UCHAR(false);
