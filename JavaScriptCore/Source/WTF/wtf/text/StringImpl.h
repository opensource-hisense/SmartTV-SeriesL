...
#define REFSTRINGIMPL()                                                 \
    if (StringImpl::sRefCallback) {                                     \
        if (m_hashAndFlags & s_hashFlag8BitBuffer) {                    \
            StringImpl::sRefCallback(this, reinterpret_cast<const char*>(m_data8), 0, m_length); \
        } else {                                                        \
            StringImpl::sRefCallback(this, 0, reinterpret_cast<const int16_t*>(m_data16), m_length); \
        }                                                               \
    }

#define DEREFSTRINGIMPL()                       \
    if (StringImpl::sDerefCallback)             \
        StringImpl::sDerefCallback(this);
#else
#define REFSTRINGIMPL()
#define DEREFSTRINGIMPL()
#endif
...
class StringImpl {
    ...
    friend class ::Netflix_StringImplCreate;
...
public:
#ifdef NRDP_JSC_ENABLE_OSALLOCATOR_CALLBACKS
    typedef void(*RefCallback)(void *, const char *, const int16_t *, unsigned);
    typedef void(*DerefCallback)(void *);
    static RefCallback sRefCallback;
    static DerefCallback sDerefCallback;
    static void setCallbacks(RefCallback refCallback, DerefCallback derefCallback)
    {
        sRefCallback = refCallback;
        sDerefCallback = derefCallback;
    }
#endif
...
public:
    ...
    WTF_EXPORT_PRIVATE size_t reverseFindLineTerminator(unsigned index = UINT_MAX) const;
