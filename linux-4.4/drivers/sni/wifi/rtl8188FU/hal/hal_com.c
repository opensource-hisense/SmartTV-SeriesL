...
static u8 rtw_hal_set_gpio_custom_cmd(_adapter *adapter, u8 enable)
{
	...
	u8 customid = 0x2, special_wakeup_reason = Rx_MagicPkt, custom_for_wakeup_reason=0x1;
	u8 ret = _FAIL;
	...
	if(enable) {
		...
		ret = rtw_hal_fill_h2c_cmd(adapter,
					   ...
					   H2C_GPIO_CUSTOM_LEN,
					   H2CGpioCustomParm);
		DBG_871X("%s(): H2C_cmd=%x, cmd=%02x, %02x, %02x\n", __func__, H2C_GPIO_CUSTOM,
...
static u8 rtw_hal_set_wowlan_ctrl_cmd(_adapter *adapter, u8 enable, u8 change_unit)
{
	...
	u8 discont_wake = 1, gpionum = 0, gpio_dur = 13;
	u8 hw_unicast = 0, gpio_pulse_cnt = 0, gpio_pulse_en = 1;
...
static void rtw_hal_ap_wow_enable(_adapter *padapter)
{
	...
	rtw_write8(padapter, REG_RSV_CTRL, 0x00);
    rtw_write8(adapter, 0x6, 0xa);
